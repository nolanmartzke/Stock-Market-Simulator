...
We plan on using react to make our user interface where users will be able to buy stocks and track performance. That interface will communicate with a java backend that will use an API to retrieve realtime market data and push it to the user. Finally we plan on using a MYSQL database to store information about our users like current and previous stock holdings.